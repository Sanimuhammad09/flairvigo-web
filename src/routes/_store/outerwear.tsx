...
export const Route = createFileRoute('/_store/outerwear')({
  component: OuterwearCollection,
})

function OuterwearCollection() {
  const { data: outerwearProducts, isLoading } = useQuery({
    queryKey: ['products', 'outerwear'],
    queryFn: async () => {
      const res = await api.get('/products?category=outerwear');
      return res.data;
    }
  });
    ...
    <main className="flex-1 bg-brand-bg">
      ...
      <section className="relative w-full h-[70vh] min-h-[500px] flex items-end justify-center pb-16">
        ...
        <div className="relative z-10 text-center text-white px-6 max-w-4xl">
          <h1 className="font-serif text-5xl md:text-7xl font-bold mb-4 tracking-tight">Outerwear Collection</h1>
          <p className="font-sans text-lg md:text-xl mb-8 max-w-2xl mx-auto opacity-90">
            Stay warm without sacrificing mobility or style. Premium jackets and vests for on and off shift.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/outerwear" className="bg-white text-brand px-10 py-4 font-bold tracking-widest text-sm uppercase rounded-full hover:bg-gray-100 transition-colors">
              Shop Tops
            </Link>
            <Link to="/outerwear" className="bg-white text-brand px-10 py-4 font-bold tracking-widest text-sm uppercase rounded-full hover:bg-gray-100 transition-colors">
              Shop Pants
      ...
      <section className="bg-white py-16 px-6 overflow-hidden border-t border-b border-brand-border">
        ...
        <div className="flex space-x-6 overflow-x-auto no-scrollbar max-w-[1600px] mx-auto pb-4">
          <Link to="/outerwear" className="flex flex-col items-center min-w-[120px] group">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 bg-white mb-16">
        <div className="flex flex-col justify-center items-center text-center p-12 md:p-24 order-2 md:order-1">
          ...
          <Link className="inline-block bg-brand text-white font-bold py-4 px-12 rounded-full hover:bg-opacity-90 transition-opacity uppercase tracking-widest text-sm" to="/outerwear">
            Shop Joggers
          </Link>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {(() => {
              const staticProducts = [
                { id: "os1", slug: "vigo-core-top", name: "The Vigo Core Top", description: "Deep Burgundy", basePrice: 48000, images: [{ isMain: true, url: "https://lh3.googleusercontent.com/aida-public/AB6AXuA0USvj4Wziq65uVnqqDe6J4TgUEejVGnGCDUNdHwfm35UKdZ962vL8w7U0uijBLKyRshw4wq3wj7OzzSW3OH0GWXioL9aniRqheGzyKJ5RsOsl2NRULrG0kEojGQuNjApCC9zIb6bStIws2LDAK2Ko88AvjmrQfHUyk6nSNezt28W9x1138diZB79V6wuQUIbbsy63bNZpaaeNXSpnYR6T2UEOLBAQW2-fo2XdnUVwOVxG1zt9xoa68g" }] },
                { id: "os2", slug: "precision-jogger", name: "Precision Jogger", description: "Charcoal", basePrice: 58000, images: [{ isMain: true, url: "https://lh3.googleusercontent.com/aida-public/AB6AXuB_Yxqa1eohBtzMP39A-_DXr3_gcH37rPbM3uZS1TIArm2XQWZ9JI5Xq9UKH2RMRFUrY24xQ-B-sdwEJcE33drauQmr0KKjB0_vwOfi3yJFa9bu0dSjzemHgPEbrvzfG3iwUH6dGmoEWKXI6fnjz_dcehJRhPhB8bVV0HsrpbWXH45Jxsfq5uG0AMuA5uQ2Ilx-_Z5lVa6109vgZV5B6quC-TkdfZeyD_o3M84NjVzpg4Q9wdl4jTm5vA" }] },
                { id: "os3", slug: "core-tech-vest", name: "The Core Tech Vest", description: "Deep Burgundy", basePrice: 85000, images: [{ isMain: true, url: "https://lh3.googleusercontent.com/aida-public/AB6AXuAhDLULQvEqksOD2VDgeHFMCAQcPiNinWBMdRtqaZLmanP-E9vuaFAQs2m5tTxE4-MzWdh13kGHsMxegsKG3p3DVrkezAzVlbflI-tPtm1DTBHsAml8ILCRXrsC0irxTJQ2SstksJH4a64bVYl1kjmaeODGCf-fo6gbd4gGlBg9jMRkh4uEh5jLyrTJv3a2C5Xi-rzM_uBV_AIlHxcLZXfvNav8hOMgK1xcSYYazUKOqz1pVGCe9dJlzw" }] }
              ];
              
              const apiProducts = outerwearProducts && Array.isArray(outerwearProducts.data || outerwearProducts) ? (outerwearProducts.data || outerwearProducts) : [];
              const productsToDisplay = apiProducts.length > 0 ? apiProducts : staticProducts;
              
              return productsToDisplay.slice(0, 6).map((product: any) => {
